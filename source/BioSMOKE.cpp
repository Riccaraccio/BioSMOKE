...
int main(int argc, char** argv) {
  ...
  // Program options from command line
  {
    ...
    description.add_options()("help", "print help messages")(
        "input", po::value<std::string>(),
        "name of the file containing the main dictionary (default \"input.dic\")")(
        "dictionary", po::value<std::string>(),
        "name of the main dictionary to be used (default \"Biomass\")");
    ...
    try {
      ...
      if (vm.count("dictionary")) main_dictionary_name_ = vm["dictionary"].as<std::string>();
  ...
  {
    {
      ...
      if (thermodynamicsMapXML->NumberOfSpecies() > max_number_allowed_species) {
        ...
        tag << max_number_allowed_species;
        OpenSMOKE::FatalErrorMessage("The OpenSMOKE++ license you are using is limited to " +
                                     tag.str() + " species");
      }
    }
    ...
    // Read the solid-phase reactions
    {
      ...
      if (!boost::filesystem::exists(path_kinetics_output / "kinetics.solid.xml")) {
        ...
        boost::property_tree::ptree ptree;
        boost::property_tree::read_xml((path_kinetics_output / "kinetics.solid.xml").string(),
                                       ptree);
        ...
        for (unsigned int k = 0; k < thermodynamicsSolidMapXML->number_of_materials(); k++)
          kineticsSolidMapXML[k] =
              new OpenSMOKE::KineticsMap_Solid_CHEMKIN(*thermodynamicsSolidMapXML, ptree, k + 1);
        // kineticsSolidMapXML = new
        // OpenSMOKE::KineticsMap_Solid_CHEMKIN(*thermodynamicsSolidMapXML, ptree, 1);
  ...
  if (dictionaries(main_dictionary_name_).CheckOption("@Analysis") == true) {
    ...
    if (typeAnalysis == "TG_Analysis") {
      ...
      if (dictionaries(main_dictionary_name_).CheckOption("@TGA"), true)
        dictionaries(main_dictionary_name_)
            .ReadDictionary("@TGA", name_of_solid_status_subdictionary);

      Get_TGAanalysisFromDictionary(dictionaries(name_of_solid_status_subdictionary), analysis,
                                    Heat_Rates, final_time, output_species_);
      ...
      Get_TotalSimulation_analysisFromDictionary(
          dictionaries(name_of_solid_status_subdictionary), analysis, energyBalance, volumeLoss,
          final_time, epsi, tau, intervalli, raggioTot, Da, output_species_);
    }

    std::string name_of_gas_status_subdictionary;
    if (dictionaries(main_dictionary_name_).CheckOption("@TemperatureProfile") == true) {
      dictionaries(main_dictionary_name_)
          .ReadDictionary("@TemperatureProfile", name_of_gas_status_subdictionary);

      OpenSMOKE::OpenSMOKEVectorDouble x, y;
      std::string x_variable, y_variable;
      GetXYProfileFromDictionary(dictionaries(name_of_gas_status_subdictionary), x, y, x_variable,
                                 y_variable);
    ...
    OpenSMOKE::FatalErrorMessage("Unknown simulation type: " + typeAnalysis +
                                 "  Conditions:TG or Total_Analysis");
  ...
  // Read initial conditions
  {
    ...
    if (dictionaries(main_dictionary_name_).CheckOption("@InletGasStatus") == true)
      dictionaries(main_dictionary_name_)
          .ReadDictionary("@InletGasStatus", name_of_gas_status_subdictionary);

    GetGasStatusFromDictionary(dictionaries(name_of_gas_status_subdictionary),
                               *thermodynamicsMapXML, Tbulk, Pbulk, rhoGas, omegaIn_gas);
  }

  {
    ...
    if (dictionaries(main_dictionary_name_).CheckOption("@InletSolidStatus") == true)
      dictionaries(main_dictionary_name_)
          .ReadDictionary("@InletSolidStatus", name_of_solid_status_subdictionary);

    GetSolidStatusFromDictionary(dictionaries(name_of_solid_status_subdictionary),
                                 *thermodynamicsSolidMapXML, Tsolid, Psolid, rhoSolid,
                                 omegaIn_solid, hext, lambdaS);

    if (temperature_profile == true) Tsolid = temperature_profile_->Get(0.);
  }

  // Calcolo il numero di specie solide, gas ed il numero di reazioni totali;

  std::cout << "Number of materials:           " << thermodynamicsSolidMapXML->number_of_materials()
            << std::endl;
  ...
  thermodynamicsMapXML->SetPressure(Pbulk);
  transportMapXML->SetPressure(Pbulk);
  thermodynamicsMapXML->SetTemperature(Tbulk);
  transportMapXML->SetTemperature(Tbulk);

  MW_gas_in = thermodynamicsMapXML->MolecularWeight_From_MassFractions(omegaIn_gas.GetHandle());
  thermodynamicsMapXML->MoleFractions_From_MassFractions(xGasIn.GetHandle(), MW_gas_in,
                                                         omegaIn_gas.GetHandle());
  ExtGas_cp =
      thermodynamicsMapXML->cpMolar_Mixture_From_MoleFractions(xGasIn.GetHandle()) / MW_gas_in;
  ...
  for (int i = 1; i <= P.Size(); i++) P[i] = Psolid;
  ...
  for (unsigned int k = 0; k < thermodynamicsSolidMapXML->number_of_materials(); k++) {
    ...
    for (int z = 0;
         z < kineticsSolidMapXML[k]->stoichiometry().stoichiometric_matrix_products().outerSize();
         ++z) {
      if (z <= (gas_species - 1)) {
        for (Eigen::SparseMatrix<double>::InnerIterator it(
                 kineticsSolidMapXML[k]->stoichiometry().stoichiometric_matrix_products(), z);
             it; ++it) {
    ...
    for (int z = 0;
         z < kineticsSolidMapXML[k]->stoichiometry().stoichiometric_matrix_reactants().outerSize();
         ++z) {
      if (z <= (gas_species - 1)) {
        for (Eigen::SparseMatrix<double>::InnerIterator it(
                 kineticsSolidMapXML[k]->stoichiometry().stoichiometric_matrix_reactants(), z);
             it; ++it) {
          nu[it.index() + 1][z + 1 + solid_species] = -it.value();
        }
      } else {
  ...
  if (typeAnalysis == "TG_Analysis") {
...

    {
      ...
      for (unsigned int i = 0; i < NSolid_; i++)
        widths_of_formation_rates_species_[i] = OpenSMOKE::CalculateSpeciesFieldWidth(
            thermodynamicsSolidMapXML->NamesOfSpecies()[i + NGas_], NSolid_);

      for (unsigned int i = NSolid_; i < NS_; i++)
        widths_of_formation_rates_species_[i] = OpenSMOKE::CalculateSpeciesFieldWidth(
            thermodynamicsMapXML->NamesOfSpecies()[i - NSolid_], NGas_);
  ...
  if (typeAnalysis == "Total_Analysis") {
      ...
      for (int i = 0; i < NS_; i++) {
        if (i < NSolid_)
          OpenSMOKE::PrintTagOnASCIILabel(
              25, yieldSpecies,
              thermodynamicsSolidMapXML->NamesOfSpecies()[i + NGas_] + "[kg/kg_s0]", counter);
        else
          OpenSMOKE::PrintTagOnASCIILabel(
              25, yieldSpecies, thermodynamicsMapXML->NamesOfSpecies()[i - NSolid_] + "[kg/kg_s0]",
              counter);
      }
...

    {
      ...
      for (int i = 0; i < NS_; i++) {
        if (i < NSolid_)
          OpenSMOKE::PrintTagOnASCIILabel(
              25, fmassTotal, thermodynamicsSolidMapXML->NamesOfSpecies()[i + NGas_] + "[kg]",
              counter);
        else
          OpenSMOKE::PrintTagOnASCIILabel(
              25, fmassTotal, thermodynamicsMapXML->NamesOfSpecies()[i - NSolid_] + "[kg]",
              counter);
      }
    ...
    /*{
        ...
        for (unsigned int i = 0; i < NS_; i++)
            widths_of_formation_rates_species_[i] =
    OpenSMOKE::CalculateSpeciesFieldWidth(thermodynamicsMapXML->NamesOfSpecies()[i], NS_);

        for (int i = 0; i < NS_; i++)
        {
            OpenSMOKE::PrintTagOnASCIILabel(25, fmassTotal,
    thermodynamicsMapXML->NamesOfSpecies()[i] + "[kg]", counter);
        }
...

    {
      ...
      for (int i = 0; i < NS_; i++) {
        OpenSMOKE::PrintTagOnASCIILabel(
            25, gasFlowRate, thermodynamicsMapXML->NamesOfSpecies()[i] + "[kg/s]", counter);
      }
  ...
  if (typeAnalysis == "TG_Analysis") {
    ...
    for (int j = 0; j < (solid_species); j++) massTotsolid_initial += x0[j];
    ...
    for (int i = 1; i <= intervalli; i++) epsi_var[i] = epsi;

    // Initial Conditions
    // Il vettore delle x0=m_0_j � cos� definito per l'i-esimo intervallo:
    ...
    for (int i = 1; i <= intervalli; i++) {
      for (int j = 0; j < Neq; j++) {
        if (j < solid_species)
          ...
          x0[j + Neq * (i - 1)] =
              omegaIn_gas[j + 1 - solid_species] * rhoGas_internal * V[i] * epsi_var[i];
        else
          x0[j + Neq * (i - 1)] = Tsolid;
      }
    }
    ...
    massTotsolid_initial = 0.;
    for (int i = 1; i <= intervalli; i++) {
      for (int j = 0; j < (solid_species); j++) massTotsolid_initial += x0[j + Neq * (i - 1)];
    }
