...
void OdeTGA(const double* y, const double t, double* dy, void* args) {
  ...
  if (temperature_profile == true) Heat_Rates = temperature_profile_->Get_increase(t);
  ...
  massFinalSolid = 0.;
  for (int j = 1; j <= (solid_species); j++) massFinalSolid += mass_TGA[j];
  ...
  for (int j = 1; j <= (solid_species); j++) omegaSolid_TGA[j] = mass_TGA[j] / (massFinalSolid);
  ...
  for (int j = 1; j <= gas_species; j++) {
    cGas_TGA[j] = Psolid / 8.314 / T_TGA *
                  xGasIn[j];  // Cgas calcualted using fixed gas composition = inlet gas composition
  }

  OpenSMOKE::OpenSMOKEVectorDouble cSolid_TGA(solid_species);
  for (int j = 1; j <= solid_species; j++) {
    cSolid_TGA[j] =
        rhoSolid * omegaSolid_TGA[j] /
        MW_tot[j];  // LOCASPI NEW FORUMLA, more general but needs correction therm in the dy eqs
  ...
  for (int i = 0; i < Neq; i++) {
    if (i < solid_species)
      dy[i] = RSolid[i + 1] * MW_tot[i + 1] * massFinalSolid /
              rhoSolid;  // *massFinalSolid/rhoSolid not needed for Gentile's old formula
...
void MyStepPrintTGA(const double t, const Eigen::VectorXd& Y) {
  ...
  if (count_ode_video_ % n_steps_video_ == 1 || t == final_time) {
    ...
    std::cout << std::scientific << std::setw(18) << std::setprecision(6) << std::left
              << massFinalSolid / massTotsolid_initial;
    std::cout << std::endl;
  }

  if (count_file_ == n_steps_file_ || t == final_time) {
    TG << std::setw(25) << std::left << t << std::setw(25) << std::left << t / 60 << std::setw(25)
       << std::left << T_TGA - 273.15 << std::setw(25) << std::left << T_TGA << std::setw(25)
       << std::left << massFinalSolid / massTotsolid_initial << std::endl;

    yieldSpecies << std::setw(25) << std::left << t << std::setw(25) << std::left << t / 60
                 << std::setw(25) << std::left << T_TGA - 273.15 << std::setw(25) << std::left
                 << T_TGA;
  ...
  if (t == final_time) {
    for (int j = 1; j <= NSolid_; j++) {
      finalYield << std::setw(35) << std::left
                 << thermodynamicsSolidMapXML->NamesOfSpecies()[j + NGas_ - 1] << std::setw(35)
                 << std::left << omegaTot_TGA[j] << std::endl;
    }

    for (int j = NSolid_ + 1; j <= NS_; j++) {
      finalYield << std::setw(35) << std::left
                 << thermodynamicsSolidMapXML->NamesOfSpecies()[j - NSolid_ - 1] << std::setw(35)
                 << std::left << omegaTot_TGA[j] << std::endl;
    }

    for (int j = 1; j <= NSolid_; j++) {
      finalYieldSolid << std::setw(35) << std::left
                      << thermodynamicsSolidMapXML->NamesOfSpecies()[j + NGas_ - 1] << std::setw(35)
                      << std::left << omegaSolid_TGA[j] << std::endl;
    }
    for (int j = 1; j <= NGas_; j++) {
      finalYieldGas << std::setw(35) << std::left
                    << thermodynamicsSolidMapXML->NamesOfSpecies()[j - 1] << std::setw(35)
                    << std::left << omegaGas_TGA[j] << std::endl;
    }
    ...
    for (int j = 0; j < residualIndex.size(); j++) solidFinal += omegaTot_TGA[residualIndex[j]];

    for (int j = 0; j < lightGasIndex.size(); j++) gasFinal += omegaTot_TGA[lightGasIndex[j]];

    for (int j = 0; j < tarIndex.size(); j++) tarFinal += omegaTot_TGA[tarIndex[j]];

    tarFinal = 1 - solidFinal - gasFinal;

    yieldScheletal << std::setw(35) << std::left << "CHAR  " << std::setw(35) << std::left
                   << solidFinal << std::endl;
    yieldScheletal << std::setw(35) << std::left << "LIGHT GAS  " << std::setw(35) << std::left
                   << gasFinal << std::endl;
    yieldScheletal << std::setw(35) << std::left << "TAR  " << std::setw(35) << std::left
                   << tarFinal << std::endl;
  }
}
...
void OdeTotal(const double* y, const double t, double* dy, void* args) {
  ...
  // T[intervalli] = 473.15;
  // T[intervalli] = 866 - (866 - 300)*exp(-t / 50) + 700 * (1 - exp(-t / 50))*exp(-t / 25); //BEST
  // CORRELATION FOR PARK T[intervalli] = 742-(742-300)*exp(-t/50)+500*(1-exp(-t/20))*exp(-t/25);
  ...
  for (int i = 1; i <= intervalli; i++)
    for (int j = 1; j <= (solid_species + gas_species); j++) mole[i][j] = mass[i][j] / MW_tot[j];
  ...
  for (int i = 1; i <= (intervalli * Neq); i++) residual[i] = dy[i - 1];

  norm2 = residual.Norm2();

  massFinalSolid = 0.;
  for (int i = 1; i <= intervalli; i++) {
    for (int j = 1; j <= (solid_species); j++) massFinalSolid += mass[i][j];
  }
  ...
  for (int i = 1; i <= intervalli; i++) {
    massGasPerfect +=
        P[i] * V[i] * epsi_var[i] * MW_gas_mix[i] / PhysicalConstants::R_J_kmol / T[i];
  }
  ...
  {
      residual_CELL[i] =
  massFraction[i][thermodynamicsSolidMapXML->IndexOfSpecies("CELL")-gas_species]; residual_CELLA[i]
  = massFraction[i][thermodynamicsSolidMapXML->IndexOfSpecies("CELLA")-gas_species];
      residual_CHAR[i] =
  massFraction[i][thermodynamicsSolidMapXML->IndexOfSpecies("CHAR")-gas_species]; residual_ASH[i] =
  massFraction[i][thermodynamicsSolidMapXML->IndexOfSpecies("ASH")-gas_species];
      //residual_TANN[i] =
  massFraction[i][thermodynamicsSolidMapXML->IndexOfSpecies("TANN")-gas_species];
      //residual_TGL[i] =
  massFraction[i][thermodynamicsSolidMapXML->IndexOfSpecies("TGL")-gas_species];
...
void MyStepPrintTotal(const double t, const Eigen::VectorXd& x) {
  ...
  for (int i = 1; i <= intervalli; i++) {
    for (int j = 1; j <= (solid_species + gas_species); j++) {
      if (j <= solid_species) {
        ...
        massSolidSpeciesTotal_fromR[j] += (t - t_old) *
                                          (FormationRate[i][j] + FormationRate_old[i][j]) / 2. *
                                          MW_tot[j] * V[i] * (1 - epsi_var[i]);
      } else {
        massGasSpeciesTotal[j - solid_species] += mass[i][j];
        massGasSpeciesTotal_fromR[j - solid_species] +=
            (t - t_old) * (FormationRate[i][j] + FormationRate_old[i][j]) / 2. * MW_tot[j] * V[i] *
            (1 - epsi_var[i]);
      }

      /*if (count_file_ == n_steps_file_ && j>solid_species)
          std::cout << FormationRate[i][j] << " " << FormationRate_old[i][j] << " " << (t - t_old) *
         (FormationRate[i][j] + FormationRate_old[i][j]) / 2. << " " << massGasSpeciesTotal_fromR[j
         - solid_species] << std::endl;*/
  ...
  for (int j = 1; j <= gas_species; j++) {
    massGasSpeciesFlowrate[j] =
        massConvection_out[intervalli] * massFraction[intervalli][j + solid_species] +
        JD_out[intervalli][j];

    massGasSpecies_outIntegrated[j] +=
        (t - t_old) * (massGasSpeciesFlowrate_old[j] + massGasSpeciesFlowrate[j]) / 2.;
    tot += massGasSpecies_outIntegrated[j];
  }

  t_old = t;
  for (int j = 1; j <= (solid_species + gas_species); j++) {
    for (int i = 1; i <= intervalli; i++) FormationRate_old[i][j] = FormationRate[i][j];
    if (j > solid_species)
  ...
  if (count_file_ == n_steps_file_ || t == final_time) {
    ...
    QreactTot << std::setw(25) << std::left << t << std::setw(25) << std::left << HeatReactionTot
              << "\n";
    ...
    volume << std::setw(25) << std::left << t << std::setw(25) << std::left << Vtot / Vtot_iniz
    ...
    TG << std::setw(25) << std::left << t << std::setw(25) << std::left
       << massFinalSolid / massTotsolid_initial << "\n";
    ...
    yieldSpecies << std::setw(25) << std::left
                 << (massSolidSpeciesTotal_fromR.SumElements() +
                     massGasSpeciesTotal_fromR.SumElements()) /
                        massTotsolid_initial;

    for (int j = 1; j <= solid_species; j++)
      yieldSpecies << std::setw(25) << std::left
                   << massSolidSpeciesTotal_fromR[j] / massTotsolid_initial;

    for (int j = 1; j <= gas_species; j++)
      yieldSpecies << std::setw(25) << std::left
                   << massGasSpeciesTotal_fromR[j] / massTotsolid_initial;
    ...
    for (int j = 1; j <= gas_species; j++)
      fmassTotal << std::setw(25) << std::left
                 << (massGasSpecies_outIntegrated[j] + massGasSpeciesTotal[j]) /
                        massTotsolid_initial;
    ...
    for (int k = 0; k < output_species_.size(); k++) {
      ...
      if (index <= NGas_) {
        for (int j = 1; j <= intervalli; j++) {
          ...
          // species[k] << std::setw(25) << std::left << massFraction_Shellmassbased[j][index +
          // NSolid_];
        }
      } else {
        for (int j = 1; j <= intervalli; j++) {
...
void SetVolume(const double t) {
  if (volumeLoss == true) {
    ...
    for (int i = 1; i <= intervalli; i++) V[i] = massTot_solid[i] / (rhoSolid * (1 - epsi_var[i]));
    ...
    if (intervalli > 1) {
      for (int i = 2; i <= intervalli; i++) r[i] = (raggio[i] + raggio[i - 1]) / 2.;
    }

    for (int i = 1; i <= intervalli; i++) S[i] = 4. * PhysicalConstants::pi * pow(raggio[i], 2.);

    Vtot = V.SumElements();

    for (int i = 1; i <= intervalli; i++) Ugrid[i] = (Rstart[i] - raggio[i]) / (t);
...
void CalculateProperties() {
  if (volumeLoss == false) {
    for (int i = 1; i <= intervalli; i++) epsi_var[i] = 1 - massTot_solid[i] / V[i] / rhoSolid;
  }

  for (int i = 1; i <= intervalli; i++) {
    // Proprietà del gas
    for (int j = 1; j <= gas_species; j++) omegaGasFraction[j] = massFraction[i][j + solid_species];

    for (int j = 1; j <= solid_species; j++) omegaSolidFraction[j] = massFraction[i][j];

    thermodynamicsMapXML->MoleFractions_From_MassFractions(xGasFraction.GetHandle(), MW_gas,
                                                           omegaGasFraction.GetHandle());
    MW_gas_mix[i] = MW_gas;

    P[i] =
        massTot_gas[i] / MW_gas_mix[i] / (V[i] * epsi_var[i]) * PhysicalConstants::R_J_kmol * T[i];
    ...
    cPgas_mix[i] = thermodynamicsMapXML->cpMolar_Mixture_From_MoleFractions(
        xGasFraction.GetHandle());  //[J/Kmol/K]
    ...
    for (int j = 1; j <= gas_species; j++) Dmix[i][j] = Diff[j];
    ...
    for (int j = 1; j <= gas_species; j++)
      // Diff_eff[i][j] =
      // epsi_var[i]/tau*((Dmix[i][j]*Diff_Knudsen[i][j])/(Dmix[i][j]+Diff_Knudsen[i][j]));
    ...
    {
        std::cout << thermodynamicsSolidMapXML->NamesOfSpecies()[j-1] << "\t" <<
    Hi_tot[j]/1000/4.186 << "\t" << Si_tot[j]/1000/4.186 << endl;
    ...
    for (int j = 1; j <= gas_species + solid_species; j++) Hi_mass_tot[i][j] = Hi_tot_corr[j];

    C_gas[i] = P[i] / (PhysicalConstants::R_J_kmol * T[i]);  // moleTot_gas[i]/(V[i]*epsi_var[i]);

    rho_gas[i] = P[i] * MW_gas_mix[i] /
                 (PhysicalConstants::R_J_kmol * T[i]);  // moleTot_gas[i]/(V[i]*epsi_var[i]);
...
void CalculateKinetics() {
  ...
  for (int i = 1; i <= intervalli; i++) {
    ...
    for (int j = 1; j <= solid_species; j++) omegaSolidFraction[j] = massFraction[i][j];
    thermodynamicsMapXML->MoleFractions_From_MassFractions(xGasFraction.GetHandle(), MWgas,
                                                           omegaGasFraction.GetHandle());
    ...
    for (unsigned int k = 0; k < thermodynamicsSolidMapXML->number_of_materials(); k++) {
      ...
      {
        /*thermodynamicsMapXML->MoleFractions_From_MassFractions(xGasFraction.GetHandle(), MWgas,
        omegaGasFraction.GetHandle()); cTotGas_ = 1000000000.; Product(cTotGas_, xGasFraction,
        &cGas);*/

        double MWsolid = 0.;
        thermodynamicsSolidMapXML->SolidMoleFractions_From_SolidMassFractions(
            xSolidFraction.GetHandle(), MWsolid, omegaSolidFraction.GetHandle());
        ...
        OpenSMOKE::OpenSMOKEVectorDouble RSolid(
            thermodynamicsSolidMapXML->number_of_solid_species());
        ...
        sumRsolid[i] = 0.0;
        for (int j = 1; j <= gas_species; j++) sumRsolid[i] += RSolid[j] * MW_solid[j];

        // std::cout << sumRgas[i] << "\t" << sumRsolid[i] << "\t" << sumRgas[i]+ sumRsolid[i] <<
        // endl; getchar();
        // HeatReaction[i] = kineticsSolidMapXML[k]->HeatRelease(RGas, RSolid)*V[i]*(1-epsi_var[i]);
        // // W

        HeatReaction[i] +=
            kineticsSolidMapXML[k]->HeatRelease(RGas.GetHandle(), RSolid.GetHandle()) * V[i] *
            (1 - epsi_var[i]);  // W
        ...
        for (int j = 1; j <= solid_reactions; j++)
          for (int k = 1; k <= gas_species; k++) Vel_gas[i][k] += nu[j][k + solid_species] * RGG[j];
...
void CreateMaterialBalances() {
  ...
  if (intervalli > 1) {
    for (int i = 1; i <= (solid_species + gas_species); i++) {
      if (i <= solid_species) {
        ...
        massConvection_out[1] =
            -Da / dyn_viscosity[1] * rho_gas[1] * S[1] * (P[2] - P[1]) / (r[2] - r[1]);

        JD_in[1][i - solid_species] = 0.;
        JD_out[1][i - solid_species] = -Diff_eff[1][i - solid_species] * rho_gas[1] * S[1] *
                                       (massFraction[2][i] - massFraction[1][i]) / (r[2] - r[1]);

        dmass[1][i] = JD_in[1][i - solid_species] - JD_out[1][i - solid_species] +
                      (FormationRate[1][i] * MW_tot[i] * V[1] * (1 - epsi_var[1])) -
                      massConvection_out[1] * massFraction[1][i];
      }
    ...
    for (int i = 2; i <= intervalli - 1; i++) {
      for (int j = 1; j <= (solid_species + gas_species); j++) {
        if (j <= solid_species) {
          ...
          massConvection_out[i] =
              -Da / dyn_viscosity[i] * rho_gas[i] * S[i] * (P[i + 1] - P[i]) / (r[i + 1] - r[i]);

          JD_in[i][j - solid_species] = JD_out[i - 1][j - solid_species];
          JD_out[i][j - solid_species] = -Diff_eff[i][j - solid_species] * rho_gas[i] * S[i] *
                                         (massFraction[i + 1][j] - massFraction[i][j]) /
                                         (r[i + 1] - r[i]);

          dmass[i][j] = JD_in[i][j - solid_species] - JD_out[i][j - solid_species] +
                        (FormationRate[i][j] * MW_tot[j] * V[i] * (1 - epsi_var[i])) +
                        massConvection_in[i] * massFraction[i - 1][j] -
                        massConvection_out[i] * massFraction[i][j];
        }
      }
    }
  }
  ...
  for (int i = 1; i <= gas_species; i++) {
    omegaI[i] = (Kc[i] * omegaIn_gas[i] + Diff_eff[intervalli][i] *
                                              massFraction[intervalli][i + solid_species] /
                                              (raggio[intervalli] - r[intervalli])) /
  ...
  Tsurf_solid = (hext * Tbulk + lambda_effective[intervalli] * T[intervalli] /
                                    (raggio[intervalli] - r[intervalli])) /
  ...
  for (int i = 1; i <= (solid_species + gas_species); i++) {
    if (i <= solid_species) {
      ...
      massConvection_out[intervalli] = -Da / dyn_viscosity[intervalli] * rho_gas[intervalli] *
                                       S[intervalli] * (Psolid - P[intervalli]) /
                                       (raggio[intervalli] - r[intervalli]);

      JD_out[intervalli][i - solid_species] =
          -Kc[i - solid_species] * S[intervalli] * rhoGasI *
          (omegaIn_gas[i - solid_species] - omegaI[i - solid_species]);

      dmass[intervalli][i] =
          JD_in[intervalli][i - solid_species] - JD_out[intervalli][i - solid_species] +
          (FormationRate[intervalli][i] * MW_tot[i] * V[intervalli] * (1 - epsi_var[intervalli])) +
          (massConvection_in[intervalli] * massFraction[intervalli - 1][i] -
           massConvection_out[intervalli] * massFraction[intervalli][i]);
...
void CreateEnergyBalances() {
  if (energyBalance == true) {
    ...
    for (int i = 1; i <= (gas_species); i++) {
      ...
      JDH_in[1] = 0.;
      // JDH_out[1] +=
      // Hi_mass[1][i]*(JD_out[1][i]+massConvection_out[1]*massFraction[1][i+solid_species]);
    }

    JC_in[1] = 0.;
    // JC_out[1] =
    // -lambda_solid[1]*(1-epsi_var[1])*S[1]*(T[2]-T[1])/(r[2]-r[1])-lambda_gas[1]*S[1]*epsi_var[1]*(T[2]-T[1])/(r[2]-r[1]);
    JC_out[1] = -lambda_effective[1] * S[1] * (T[2] - T[1]) / (r[2] - r[1]);

    dT[1] = (-(flux_in[1] - flux_out[1]) + HeatReaction[1] + JC_in[1] - JC_out[1] + JDH_in[1] -
             JDH_out[1] /*+PhysicalConstants::R_J_kmol*T[1]*sumMoleFormationRates[1]*/) /
            ((massTot_solid[1] * cPsolid_mix[1]) +
             (massTot_gas[1] *
              cPgas_mix[1]) /*-PhysicalConstants::R_J_kmol*C_gas[1]*V[1]*epsi_var[1]*/);

    for (int i = 2; i <= intervalli - 1; i++) {
      for (int j = 1; j <= (gas_species); j++) {
        flux_in[i] += Hi_mass[i][j] *
                      (JD_in[i][j] + massConvection_in[i] * massFraction[i - 1][j + solid_species]);
        // flux_out[i] +=
        // Hi_mass[i][j]*(JD_out[i][j]+massConvection_out[i]*massFraction[i][j+solid_species]);

        JDH_in[i] += Hi_mass[i - 1][j] *
                     (JD_in[i][j] + massConvection_in[i] * massFraction[i - 1][j + solid_species]);
        // JDH_out[i] +=
        // Hi_mass[i][j]*(JD_out[i][j]+massConvection_out[i]*massFraction[i][j+solid_species]);
      }
      ...
      // JC_in[i] =
      // -lambda_solid[i-1]*S[i-1]*(1-epsi_var[i])*(T[i]-T[i-1])/(r[i]-r[i-1])-lambda_gas[i-1]*epsi_var[i]*S[i-1]*(T[i]-T[i-1])/(r[i]-r[i-1]);
      // JC_out[i] =
      // -lambda_solid[i]*(1-epsi_var[i])*S[i]*(T[i+1]-T[i])/(r[i+1]-r[i])-lambda_gas[i]*S[i]*epsi_var[i]*(T[i+1]-T[i])/(r[i+1]-r[i]);
      JC_in[i] = -lambda_effective[i - 1] * S[i - 1] * (T[i] - T[i - 1]) / (r[i] - r[i - 1]);
      JC_out[i] = -lambda_effective[i] * S[i] * (T[i + 1] - T[i]) / (r[i + 1] - r[i]);

      dT[i] = (-(flux_in[i] - flux_out[i]) + HeatReaction[i] + JC_in[i] - JC_out[i] + JDH_in[i] -
               JDH_out[i] /*+PhysicalConstants::R_J_kmol*T[i]*sumMoleFormationRates[i]*/) /
              ((massTot_solid[i] * cPsolid_mix[i]) +
               (massTot_gas[i] *
                cPgas_mix[i]) /*- PhysicalConstants::R_J_kmol*C_gas[i]*V[i]*epsi_var[i]*/);
    }
    ...
    for (int i = 1; i <= (gas_species); i++) {
      // flux_out[intervalli] +=
      // Hi_mass[intervalli][i]*(JD_out[intervalli][i]+massConvection_out[intervalli]*massFraction[intervalli][i+solid_species]);
      flux_in[intervalli] +=
          Hi_mass[intervalli][i] *
          (JD_in[intervalli][i] +
           massConvection_in[intervalli] * massFraction[intervalli - 1][i + solid_species]);

      JDH_in[intervalli] +=
          Hi_mass[intervalli - 1][i] *
          (JD_in[intervalli][i] +
           massConvection_in[intervalli] * massFraction[intervalli - 1][i + solid_species]);
      // JDH_out[intervalli] +=
      // Hi_mass[intervalli][i]*(JD_out[intervalli][i]+massConvection_out[intervalli]*massFraction[intervalli][i+solid_species]);
    ...
    // JC_in[intervalli] =
    // -lambda_solid[intervalli-1]*(1-epsi_var[i]_var[intervalli-1])*S[intervalli-1]*(T[intervalli]-T[intervalli-1])/(r[intervalli]-r[intervalli-1])-lambda_gas[intervalli-1]*S[intervalli-1]*epsi_var[i]_var[intervalli-1]*(T[intervalli]-T[intervalli-1])/(r[intervalli]-r[intervalli-1]);
    // JC_out[intervalli] =
    // -lambda_solid[intervalli]*(1-epsi_var[i]_var[intervalli])*S[intervalli]*(Tsup_solid-T[intervalli])/(raggio[intervalli]-r[intervalli])-lambda_gas[intervalli]*S[intervalli]*epsi_var[i]_var[intervalli]*(Tsup_solid-T[intervalli])/(raggio[intervalli]-r[intervalli]);
    JC_in[intervalli] = -lambda_effective[intervalli - 1] * S[intervalli - 1] *
                        (T[intervalli] - T[intervalli - 1]) / (r[intervalli] - r[intervalli - 1]);
    JC_out[intervalli] = -lambda_effective[intervalli] * S[intervalli] *
                         (Tsurf_solid - T[intervalli]) / (raggio[intervalli] - r[intervalli]);

    // dT[intervalli] =
    // (-(flux_in[intervalli]-flux_out[intervalli])+HeatReaction[intervalli]+JC_in[intervalli]-JC_out[intervalli]+JDH_in[intervalli]-JDH_out[intervalli]/*+PhysicalConstants::R_J_kmol*T[intervalli]*sumMoleFormationRates[intervalli]*/)
    //           /((massTot_solid[intervalli]*cPsolid_mix[intervalli]) +
    //           (massTot_gas[intervalli]*cPgas_mix[intervalli])/*-
    //           PhysicalConstants::R_J_kmol*C_gas[intervalli]*V[intervalli]*epsi_var[intervalli]*/);

    dT[intervalli] =
        (-(flux_in[intervalli] - flux_out[intervalli]) + HeatReaction[intervalli] +
         JC_in[intervalli] - JC_out[intervalli] + JDH_in[intervalli] - JDH_out[intervalli]) /
        ((massTot_solid[intervalli] * cPsolid_mix[intervalli]) +
         (massTot_gas[intervalli] * cPgas_mix[intervalli]));
    ...
    for (int i = 1; i <= intervalli; i++) dT[i] = Heat_Rates;
  }
}
